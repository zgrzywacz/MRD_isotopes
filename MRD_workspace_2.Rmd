...
```{r}
monthtest <- subset(clim, month ==5)
plot(monthtest$temp, monthtest$rad)
```
...
```{r}
ERA_t2m_sub <- subset(ERA_t2m, 121:828)
ERA_precip_sub <- subset(ERA_precip, 133:840)
```
...
```{r}
datC <- brick("rasters/ERA5_temp.gri")
  datC_curr <- subset(datC, 121:828)
  datC_next <- subset(datC, 133:840)
...
r <- brick(a, xmn=0.125, xmx=180.125, ymn=-89.875, ymx=0.125, crs=CRS("+proj=longlat +datum=WGS84"))
writeRaster(r, paste0("temp_corr_test_curr", month))

...
```{r}
gnip_filled <- read.csv("CG_GNIP_filled.csv")
gnip_filled$Date <- parse_date_time(gnip_filled$Date, "mdy")
gnip_filled$month <- month(gnip_filled$Date)
gnip_filled$year <- year(gnip_filled$Date)
for (i in c(1:288)){
if (gnip_filled$month[i] < 9){
  gnip_filled$seas.year[i] = (gnip_filled$year[i] - 1)
}
else{
  gnip_filled$seas.year[i] = gnip_filled$year[i]
}
}
```

```{r}
annual_gnip <- subset(gnip_filled, month == c(1,2,9,10,11,12))[-(1:2), , drop = FALSE]
gnip_summary <- annual_gnip %>% group_by(seas.year) %>% summarise(mean = mean(O18, na.rm=TRUE))
gnip_summary
```
...
```{r}
mean(glk(tab, overlap=10)$glk_mat, na.rm=TRUE)
```


```{r}
mean(glk(new_c13[,-4], overlap=10)$glk_mat, na.rm=TRUE)
```
...
```{r}
detrend.series(MRD_dfstd$MRD619A, method="Spline")
```
...
```{r}
o18_var <- apply(tab, MARGIN = 2, sd, na.rm=TRUE)
...
slope <- c(35,35,20,20,30,28,20,40,35,38)
dbh <- c(72,75,91,24,86,39,43,65,83,70)
altitude <- c(936.458,942.727,939.455,939.455,941.111,954.136,952.022,944.875,953.187,942.939)
o18_appendix_plots <- data.frame(mean = o18_means, sd = o18_var, soil_depth = soildepths, canopy = canopycovers, slope = slope, dbh = dbh, alt = altitude)
```

```{r}
plot(soildepths, o18_var)
plot(canopycovers, o18_var)
plot(slope, o18_var)
plot(dbh, o18_var)
plot(altitude, o18_var)
```

```{r}
cor.test(soildepths, o18_var)
cor.test(canopycovers, o18_var)
cor.test(slope, o18_var)
cor.test(dbh, o18_var)
cor.test(altitude, o18_var)
```

```{r}
plot(soildepths, o18_means)
plot(canopycovers, o18_means)
plot(slope, o18_means)
plot(dbh, o18_means)
plot(altitude, o18_means)
```

```{r}
cor.test(soildepths, o18_means)
cor.test(canopycovers, o18_means)
cor.test(slope, o18_means)
cor.test(dbh, o18_means)
cor.test(altitude, o18_means)
```

```{r}
plot(o18_means, o18_var)
cor.test(o18_means, o18_var)
```

```{r}
p1 <- ggplot(o18_appendix_plots, aes(soil_depth, mean))+
  geom_point(shape=23, color="red")+
  xlab("Soil Depth (mm)")+
  ylab("Mean d18OTR")
p2 <- ggplot(o18_appendix_plots, aes(alt, mean))+
  geom_point(shape=23, color="blue")+
  xlab("Altitude (m)")+
  ylab("Mean d18OTR")
p3 <- ggplot(o18_appendix_plots, aes(dbh, mean))+
  geom_point(shape=23, color="green")+
  xlab("Diameter at Breast Height (cm)")+
  ylab("Mean d18OTR")
p4 <- ggplot(o18_appendix_plots, aes(slope, mean))+
  geom_point(shape=23, color="orange")+
  xlab("Slope (degrees)")+
  ylab("Mean d18OTR")
p5 <- ggplot(o18_appendix_plots, aes(canopy, mean))+
  geom_point(shape=23, color="purple")+
  xlab("Canopy Cover (%)")+
  ylab("Mean d18OTR")
p6 <- ggplot(o18_appendix_plots, aes(soil_depth, sd))+
  geom_point(shape=23, color="red")+
  xlab("Soil Depth (mm)")+
  ylab("d18OTR Standard Deviation")
p7 <- ggplot(o18_appendix_plots, aes(alt, sd))+
  geom_point(shape=23, color="blue")+
  xlab("Altitude (m)")+
  ylab("d18OTR Standard Deviation")
p8 <- ggplot(o18_appendix_plots, aes(dbh, sd))+
  geom_point(shape=23, color="green")+
  xlab("Diameter at Breast Height (cm)")+
  ylab("d18OTR Standard Deviation")
p9 <- ggplot(o18_appendix_plots, aes(slope, sd))+
  geom_point(shape=23, color="orange")+
  xlab("Slope (degrees)")+
  ylab("d18OTR Standard Deviation")
p10 <- ggplot(o18_appendix_plots, aes(canopy, sd))+
  geom_point(shape=23, color="purple")+
  xlab("Canopy Cover (%)")+
  ylab("d18OTR Standard Deviation")
```
...
```{r}
png(filename="appendixplots2.png", height=800, width=2000)
...
```{r}
rwidths <- read.csv("indices_1960_2018.csv")
rownames(rwidths) <- rwidths[,1]
rwidths[,1] <- NULL
```

```{r}
rowMeans(rwidths, na.rm=TRUE)
```

```{r}
library(boot)
ci.func.mean <- function(y){
  y <- y[!is.na(y)]
  med.fun <- function(y,i)
  {
    m = mean(y[i])
...
```{r}
ci3 <- as.data.frame(t(apply(rwidths, 1, ci.func.mean)))
ci3$med <- as.numeric("")
ci3$med <- trimMRD$mrdstd
ci3$year <- as.numeric(rownames(ci3))
```
...
p<-ggplot(data=ci3, aes(x=year, group=1)) +
  ...
  geom_ribbon(aes(ymin=lower, ymax=upper, fill="95% confidence interval of mean ring width"), linetype=2, alpha=0.3)+
  geom_line(aes(y=med),size=2, col="darkslategrey")+
  labs(y= "KBP Ring Width", x = "Year")+
  scale_x_continuous(breaks=seq(1960,2020,10))+
  scale_fill_manual("",values="darkgreen")+
  ...
  scale_linetype(name = NULL)
png(filename="rwdseries.png", height=700, width=1200)
p
dev.off()
```

```{r}
mean(glk(rwidths, overlap=40)$glk_mat, na.rm=TRUE)
```

```{r}
rangetest <- apply(tab, 1, range, na.rm=TRUE)
```

```{r}
sd(rangetest[2,] - rangetest[1,])
```
...
```{r}
datY <- brick("rasters/results/seasonal/t2m_sep_nov_mean.gri")
datY <- subset(datY, 20:43)
ar.datY <- as.array(datY)

z <- apply(ar.datY, 1:2, rcorr, gnip_summary$mean, type="pearson")
corr = array(0, dim=c(360,720) )
sig = array(0, dim=c(360,720) )
for (x in 1:259200)
{
...
r <- brick(a, xmn=0.125, xmx=180.125, ymn=-89.875, ymx=0.125, crs=CRS("+proj=longlat +datum=WGS84"))
writeRaster(r, paste0("t2m_gnip_corr_sep_nov"))
```

```{r}
resbrick <- brick("precip_gnip_corr_sep_nov.gri")
```
...
```{r}
full <- brick("precip_gnip_corr_sep_feb.gri")
sepnov <- brick("t2m_gnip_corr_sep_nov.gri")
decfeb <- brick("t2m_gnip_corr_dec_feb.gri")
```
...
png(filename="gnip_precip_spatial.png", height=1200, width=1600)
```{r}
colr <- colorRampPalette(brewer.pal(9, 'RdBu'))

levelplot(masked, margin=FALSE, xlab="Longitude", ylab="Latitude", main="Correlation (1979 - 2002)", pretty=FALSE, col.regions=colr, at=seq(-0.8, 0.8, 0.1))+
  latticeExtra::layer(sp.lines(world_outline))
```

```{r}
o18_df <- data.frame(std_summary)
o18_df$std_summary <- as.numeric(o18_df$std_summary)
```

```{r}
library(dplyr)
Poso18 <- as.numeric(rownames(slice_max(o18_df, order_by = std_summary, n=15)))
Nego18 <- as.numeric(rownames(slice_min(o18_df, order_by = std_summary, n=15)))
Poso18
Nego18
```
...
```{r}
gnip_summary
Posgnip <- slice_max(gnip_summary, order_by = mean, n=6)
Neggnip <- slice_min(gnip_summary, order_by = mean, n=6)
Posgnip
Neggnip
```
...
```{r}
library(stats)
acf(std_summary, plot=FALSE)
```


```{r}
acf(gnip_summary$mean)
```

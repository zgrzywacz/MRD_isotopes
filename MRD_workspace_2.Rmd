...
```{r}
datRas <- brick("rasters/ERA5_t2m.gri")
...
b <- brick(s)
writeRaster(b,"t2m_sept_feb_mean")
```
...
```
sel_y <- names(datC)[substr(names(datC), 2, 5) %in% "2009"]
datY <- subset(datC, sel_y, value=T)
...
```{r}
datY <- brick("t2m_sept_feb_mean.gri")
...
writeRaster(r, paste0("t2m_corr_sep_feb"))
```

```{r}
resbrick <- brick("t2m_corr_sep_feb.gri")
```

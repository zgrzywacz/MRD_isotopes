...
Or take one factor -> spit out data
...
o18_corrs <- function(df, trees){
  ...
  for (param in c(3:ncol(df))){
    tempdf <- data.frame(Clim=colnames(df[param]), Month=c(1:12), Corr="")
    for (m in 1:12){
      corrs <- rep(NA, 10)
      mon_sub <- subset(df, month == m)
...
```{r}
o18_corrs(newclim, annual_summary$mean)
```

```{r}
newdf <- o18_corrs(newclim, annual_summary$mean)
```

Make this plot work again
...
Corr w/ GNIP, VPD, scPDSI (Both whole AND groupings)

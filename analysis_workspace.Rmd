...
ggplot(data=raw, aes(x=year, y=o18corr, group=as.factor(Tree_ID))) +
  geom_line(aes(color=soil_depth))+
...
ggplot(data=raw, aes(x=year, y=c13corr, group=as.factor(Tree_ID))) +
  geom_line(aes(color=canopy_cover))+
  geom_point(aes(color=canopy_cover))+
  scale_color_gradient(low="blue", high="red")
```
...
```{r}
temp_prysm <- merge(annual_mean_o18, aug_prysm, by="year")
...
```{r}
tab_prysm$year <- rownames(tab_prysm)

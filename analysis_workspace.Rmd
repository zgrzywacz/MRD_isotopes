...
mon_annual_silo <- mon_silo %>% group_by(year) %>% summarise(mean = mean(precip, na.rm = TRUE))
...
```{r}
climcorrs(upslope_summary$mean)
```
```{r}
...
test <- cor.test(mon_annual_gnip[['mean']], trees)
corrs[m] <- test$estimate
}
return(corrs)
}
...
```{r}
slope_compare <- upslope_summary[19:42,]
```

```{r}
help <- data.frame(row.names = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"))
```

```{r}
help$up_next <- gnipcorrs2(slope_compare$mean)
```

```{r}
mon_gnip <- subset(gnip, month == 1)
mon_annual_gnip <- mon_gnip %>% group_by(year) %>% summarise(mean = mean(O18, na.rm = TRUE))
```
...
```{r}
gnipcorrs2(slope_compare$mean)
```

```{r}
gnipcorrs2(next_compare$mean)
```

I am pretty sure that gnipcorrs2 works right the way above (and the way I had it before). The plotting below can help assure that:



```{r}
mon_gnip <- subset(gnip_outlier, month == 12)
```


```{r}
plot(slope_compare$mean, mon_gnip$O18)
```
